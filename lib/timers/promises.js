...
function setTimeout(after, value, options = kEmptyObject) {
  ...
  const ret = new Promise((resolve, reject) => {
    ...
    if (signal) {
      oncancel = cancelListenerHandler.bind(timeout, clearTimeout, reject, signal);
      kResistStopPropagation ??= require('internal/event_target').kResistStopPropagation;
      signal.addEventListener('abort', oncancel, { __proto__: null, [kResistStopPropagation]: true });
    }
  });
  if (oncancel === undefined) return ret;
  return Promise.finally(ret, () => signal.removeEventListener('abort', oncancel));
}
