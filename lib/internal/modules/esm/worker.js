...
async function customizedModuleWorker(lock, syncCommPort, errorHandler) {
...

  {
    ...
    process.exit = function(code) {
      ...
      Atomics.add(lock, WORKER_TO_MAIN_THREAD_NOTIFICATION, 1);
      Atomics.notify(lock, WORKER_TO_MAIN_THREAD_NOTIFICATION);
  ...
  async function handleMessage({ method, args, port }) {
    ...
    // We are about to yield the execution with `await Reflect.apply` below. In case the code
    ...
    let response;
    try {
      response = await Reflect.apply(hooks[method], hooks, args);
    } catch (exception) {
      hasError = true;
      response = exception;
    }
