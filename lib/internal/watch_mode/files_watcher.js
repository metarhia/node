...
class FilesWatcher extends EventEmitter {
  ...
  #debounceOwners = new Set();
  #debounceTimer;
  #debounce;
  #mode;
  #signal;
  #passthroughIPC = false;
  #ipcHandlers = new WeakMap();
  ...
  filterFile(file, owner) {
    ...
    if (owner) {
      const owners = this.#dependencyOwners.get(file) ?? new Set();
      const dependencies = this.#ownerDependencies.get(file) ?? new Set();
      owners.add(owner);
  ...
  watchChildProcessModules(child, key = null) {
    ...
    child.on('message', (message) => {
      try {
        if (Array.isArray(message['watch:require'])) {
          ArrayPrototypeForEach(message['watch:require'], (file) => this.filterFile(file, key));
        }
        if (Array.isArray(message['watch:import'])) {
