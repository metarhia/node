...
function convertToInt(name, value, bitLength, options = kEmptyObject) {
  ...
  if (bitLength === 64) {
    ...
    lowerBound = !signed ? 0 : Number.MIN_SAFE_INTEGER;
  } else if (!signed) {
  ...
  if (Number.isNaN(x) || x === 0 || x === Infinity || x === -Infinity) {
    return 0;
  }
