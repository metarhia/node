...
const legacyWrapperList = new Set([
  'natives',
  'util',
]);
...
const experimentalModuleList = new Set(['sqlite']);
...
let canBeRequiredByUsersWithoutSchemeList =
  new Set(publicBuiltinIds.filter((id) => !schemelessBlockList.has(id)));
...
class BuiltinModule {
  ...
  static setRealmAllowRequireByUsers(ids) {
    canBeRequiredByUsersList =
      new Set(ArrayPrototypeFilter(ids, (id) => ArrayPrototypeIncludes(publicBuiltinIds, id)));
    canBeRequiredByUsersWithoutSchemeList =
      new Set(ArrayPrototypeFilter(ids, (id) => !schemelessBlockList.has(id)));
  }
  ...
  static getCanBeRequiredByUsersWithoutSchemeList() {
    return Array.from(canBeRequiredByUsersWithoutSchemeList);
  }

  static getSchemeOnlyModuleNames() {
    return Array.from(schemelessBlockList);
  }
