...
async function * tapReporter(source) {
  ...
  for await (const { type, data } of source) {
    switch (type) {
      ...
      case 'test:stderr':
      case 'test:stdout': {
        const lines = RegExpPrototypeSymbol.split(kLineBreakRegExp, data.message);
...
function jsToYaml(indent, name, value, seen) {
  ...
  if (typeof value !== 'object') {
    ...
    const lines = RegExpPrototypeSymbol.split(kLineBreakRegExp, value);
  ...
  if (isErrorObj) {
    ...
    if (errIsAssertion) {
      ...
      result += jsToYaml(indent, 'expected', errExpected, new Set(seen));
      result += jsToYaml(indent, 'actual', errActual, new Set(seen));
      if (errOperator) {
    ...
    if (typeof errStack === 'string') {
      ...
      ArrayPrototypeForEach(
        RegExpPrototypeSymbol.split(kLineBreakRegExp, errStack),
        (frame) => {
