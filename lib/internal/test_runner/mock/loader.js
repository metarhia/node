...
if (!$__test.mock._mockExports.has(${JSON.stringify(url)})) {
  throw new Error(${JSON.stringify(`mock exports not found for "${url}"`)});
}

const $__exports = $__test.mock._mockExports.get(${JSON.stringify(url)});
...
if (module.exports === null || typeof module.exports !== 'object') {
  throw new Error('${JSON.stringify(kBadExportsMessage)}');
}
`;
  }
  ...
  for (let i = 0; i < exportNames.length; ++i) {
    ...
    if (useESM) {
      source += `export let ${name} = $__exports.namedExports[${JSON.stringify(name)}];\n`;
    } else {
      source += `module.exports[${JSON.stringify(name)}] = $__exports.namedExports[${JSON.stringify(name)}];\n`;
    }
  }
