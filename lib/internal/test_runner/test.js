...
function stopTest(timeout, signal) {
  ...
  Object.defineProperty(deferred.promise, SymbolDispose, {
    __proto__: null,
    configurable: true,
    writable: true,
      ...
      case 'boolean':
        if (concurrency) {
          this.concurrency = parent === null ?
            Math.max(availableParallelism() - 1, 1) : Infinity;
        } else {
          this.concurrency = 1;
        }
        break;
  ...
  constructor(options) {
    ...
    try {
      ...
      this.buildSuite = SafePromisePrototypeFinally(
        PromisePrototypeThen(
          Promise.resolve(Reflect.apply(this.runInAsyncScope, this, runArgs)),
          undefined,
          (err) => {
            this.fail(new ERR_TEST_FAILURE(err, kTestCodeFailure));
          }),
