...
function formatRaw(ctx, value, recurseTimes, typedArray) {
  ...
  if (ctx.sorted) {
    ...
    if (extrasType === kObjectType) {
      ...
      Reflect.apply(ArrayPrototypeSplice, null, sorted);
    }
  }
...
function formatNumber(fn, number, numericSeparator) {
  ...
  if (integer === number) {
    if (!Number.isFinite(number) || StringPrototypeIncludes(string, 'e')) {
...
function formatArrayBuffer(ctx, value) {
  ...
  let str = StringPrototypeTrim(RegExpPrototypeSymbolReplace(
    /(.{2})/g,
    hexSlice(buffer, 0, Math.min(ctx.maxArrayLength, buffer.length)),
    '$1 ',
  ));
            ...
            case 105: { // 'i'
              ...
              if (typeof tempInteger === 'bigint') {
                ...
                tempStr = formatNumberNoColor(
                  Number.parseInt(tempInteger), inspectOptions);
              }
              break;
            }
            case 102: { // 'f'
              ...
              if (typeof tempFloat === 'symbol') {
                ...
                tempStr = formatNumberNoColor(
                  Number.parseFloat(tempFloat), inspectOptions);
              }
              break;
            }
            case 99: // 'c'
